...
class QUIC_EXPORT_PRIVATE TcpCdgSenderBytes : public SendAlgorithmInterface {
 public:
  TcpCdgSenderBytes(const QuicClock* clock, const RttStats* rtt_stats,
                      QuicPacketCount initial_tcp_congestion_window,
                      QuicPacketCount max_congestion_window,
                      QuicRandom* random,
                      QuicConnectionStats* stats);
  TcpCdgSenderBytes(const TcpCdgSenderBytes&) = delete;
  TcpCdgSenderBytes& operator=(const TcpCdgSenderBytes&) = delete;
  ~TcpCdgSenderBytes() override;
  struct QUIC_EXPORT_PRIVATE DebugState {
    explicit DebugState(const TcpCdgSenderBytes& sender);
 ...
 private:
  inline  int32_t CalculateGrad(void);
  bool CdgBackoff(int32_t grad);
  void EnterCongestionWindowReduction();
  QuicByteCount CdgGetSlowThreshold();
  struct minmax {
  union {
      struct {
          int32_t min;
  ...
  //cdg
  QuicByteCount shadow_congestion_window_;
  tcp_ca_state cong_state_;
  cdg_state cdg_state_;
  uint32_t backoff_factor_;
  uint32_t backoff_count_;
  uint32_t use_ineff_;
  uint32_t ineffective_hold_;
  struct minmax cdg_rtt_;
  struct minmax cdg_rtt_prev_;
  struct minmax cdg_gsum_;
  struct minmax gradients_[kRttWin];
  bool gfilled_;
  int tail_;
